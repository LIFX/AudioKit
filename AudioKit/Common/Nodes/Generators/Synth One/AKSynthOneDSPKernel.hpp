    ...
    struct NoteState {
        ...
        void run(int frameCount, float *outL, float *outR)
        {
            ...
            if (kernel->p[detuneLFO] == 1) {
                oscmorph2->freq += (kernel->p[detuningOffset] + 0.0) * fabs(kernel->lfo1 * kernel->p[lfo1Amplitude]);
            } else if (kernel->p[detuneLFO] == 2) {
                oscmorph2->freq += (kernel->p[detuningOffset] + 0.0) * fabs(kernel->lfo2 * kernel->p[lfo2Amplitude]);
            } else {
